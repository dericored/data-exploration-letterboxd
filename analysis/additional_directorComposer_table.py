...
df = df.explode("director").explode("composer")

table = df[['director', 'composer']].copy()
table = table[table['director'] != table['composer']]
table = pd.DataFrame(table.value_counts().reset_index(name="Count"))
table = table[table['Count'] >= 2]
table['Director-Composer'] = table['director'] + ' - ' + table['composer']
table = table[['Director-Composer', 'Count']]
...
ax = render_table(table, header_columns=0, col_width=sum(widths)/17, colWidths=widths, cellLoc='center', header_color="k")
plt.title("Frequent Collaborators (Director - Composer)", fontsize=20, y=1.05, loc='left')
plt.show()
