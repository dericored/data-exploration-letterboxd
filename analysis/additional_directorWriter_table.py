...
df = df.explode("director").explode("writer")

table = df[['director', 'writer']].copy()
table = table[table['director'] != table['writer']]
table = pd.DataFrame(table.value_counts().reset_index(name="Count"))
table = table[table['Count'] >= 2]
table['Director-Writer'] = table['director'] + ' - ' + table['writer']
table = table[['Director-Writer', 'Count']]
...
ax = render_table(table, header_columns=0, col_width=sum(widths)/17, colWidths=widths, cellLoc='center', header_color="r")
plt.title("Frequent Collaborators (Director - Writer)", fontsize=20, y=1.05, loc='left')
plt.show()
